...
current_prices = price_history.iloc[-1].fillna(method='ffill').fillna(0)

# ---------------------------------- 포트폴리오 계산 ----------------------------------
p = st.session_state.portfolio.copy()
p['current_price'] = p['ticker'].map(current_prices)
p = p[p['current_price'].notna()]  # 잘못된 티커 제거
...
with tab2:
    ...
    for t in tickers:
        ...
        # 안전한 RSI 계산
        rs = gain / (loss + 1e-10)  # div0 방지
        rsi = 100 - (100 / (1 + rs))
        rsi = np.where(loss == 0, 100, rsi)  # 하락 없을 때 RSI = 100
        rsi = pd.Series(rsi, index=c.index).fillna(50)
        rsi_last = rsi.iloc[-1]
...
with tab3:
    ...
    if st.button("예측 시작"):
        ...
        df = pd.DataFrame({'ds': df['Date'], 'y': df['Close']})
        ...
        fig.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yhat_lower'], fill='tonexty', fillcolor="rgba(0,100,255,0.15)", name="80% 구간"))
